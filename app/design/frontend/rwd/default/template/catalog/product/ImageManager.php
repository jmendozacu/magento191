...
class ImageManagerCoreCustom {
    ...
    public static function thumbnail($image, $cacheImage, $size, $imageType = 'jpg', $disableCache = true, $regenerate = false) {
        ...
        if ($regenerate || !file_exists(_PS_TMP_IMG_DIR_ . $cacheImage)) {
            ...
            if ($y < $size && $x <= $maxX) {
                ...
                self::resize($image, _PS_TMP_IMG_DIR_ . $cacheImage, $ratioX, $size, $imageType);
            }
        }
    ...
    public static function checkImageMemoryLimit($image) {
        ...
        $memoryLimit = 1073741824;//Tools::getMemoryLimit();
    ...
    $sourceFile, $destinationFile, $destinationWidth = null, $destinationHeight = null, $fileType = 'jpg', $forceType = false, &$error = 0, &$targetWidth = null, &$targetHeight = null, $quality = 5, &$sourceWidth = null, &$sourceHeight = null
    ) {
        ...
        /*
        if (Configuration::get('PS_IMAGE_QUALITY') == 'png_all' || (Configuration::get('PS_IMAGE_QUALITY') == 'png' && $type == IMAGETYPE_PNG) && !$forceType) {
            $fileType = 'png';
        }*/
        ...
        if (!self::checkImageMemoryLimit($sourceFile)) {
        ...
        $srcImage = self::create($type, $sourceFile);
        if ($rotate) {
        ...
        if ($destinationWidth >= $sourceWidth && $destinationHeight >= $sourceHeight) {
            ...
            self::imagecopyresampled($destImage, $srcImage, (int) (($destinationWidth - $nextWidth) / 2), (int) (($destinationHeight - $nextHeight) / 2), 0, 0, $nextWidth, $nextHeight, $sourceWidth, $sourceHeight, $quality);
        }
        $writeFile = self::write($fileType, $destImage, $destinationFile);
    ...
    &$dstImage, $srcImage, $dstX, $dstY, $srcX, $srcY, $dstW, $dstH, $srcW, $srcH, $quality = 3
    ) {
    ...
    public static function validateUpload($file, $maxFileSize = 0, $types = null) {
        ...
        if (!self::isRealImage($file['tmp_name'], $file['type']) || !self::isCorrectImageFileExt($file['name'], $types) || preg_match('/\%00/', $file['name'])) {
    ...
    public static function validateIconUpload($file, $maxFileSize = 0) {
        if ((int) $maxFileSize > 0 && $file['size'] > $maxFileSize) {
            return sprintf(
                    Tools::displayError('Image is too large (%1$d kB). Maximum allowed: %2$d kB'), $file['size'] / 1000, $maxFileSize / 1000
            );
        }
    ...
    public static function cut($srcFile, $dstFile, $dstWidth = null, $dstHeight = null, $fileType = 'jpg', $dstX = 0, $dstY = 0) {
        ...
        $src = array(
            'width' => $srcInfo[0],
            'height' => $srcInfo[1],
            'ressource' => self::create($srcInfo[2], $srcFile),
        );
        ...
        $return = self::write($fileType, $dest['ressource'], $dstFile);
    ...
    public static function write($type, $resource, $filename) {
        ...
        if ($psPngQuality === null) {
            $psPngQuality = 7;//Configuration::get('PS_PNG_QUALITY');
        }

        if ($psJpegQuality === null) {
            $psJpegQuality = 90;//Configuration::get('PS_JPEG_QUALITY');
        }
