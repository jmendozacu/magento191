...
if ($total_pIds > 0) {
    ...
    foreach ($products as $product_id) {
		...
		<description><![CDATA[' . $pDes . ']]></description>'
            ...
            echo '===>Item ' . $i . '=>Product id: ' . $product_id . ' =>Store id: ' . $p->getStoreId() . '==>Parent Id: ' . implode(',', Mage::getModel('catalog/product_type_configurable')->getParentIdsByChild($product_id)) . "<br>";
            $i++;
        }
    }
    $xml .= '</Data>';
...
function getRelatedcolourways($product) {
    ...
    if (count($RelatedProductIds) == 0)
        return '';
    $childproducts = '<relatedcolourways>';
